...
export class PaymentMethodService {
  ...
  async create(createPaymentMethodDto: CreatePaymentMethodDto) {
    try {
      ...
      const paymentGateway: string =
        PaymentGatewayType.STRIPE.toLowerCase() as string;
  ...
  remove(id: number) {
    ...
    try {
      const { method_key } = card;
      this.stripeService.detachPaymentMethodFromCustomer(method_key);
    } catch (error) {
      console.log(error);
    }
  ...
  async makeNewPaymentMethodObject(
    ...
    const { method_key, default_card } = createPaymentMethodDto;
    const { id: user_id, name, email } = this.users[0];
